...
def draft_email(user_input, name="Utku"):
    chat = ChatOpenAI(model_name="gpt-3.5-turbo", temperature=1)
    signature = f"Kind regards, \n EmmAI, on behalf of \{name}"
    
    template = """
    
    You are a helpful assistant that drafts an email reply based on an a new email.
    
    Your goal is to help the user quickly create a direct email reply.
    
    Keep your reply consice and to the point and mimic the style of the email so you reply in a similar manner to match the tone.
